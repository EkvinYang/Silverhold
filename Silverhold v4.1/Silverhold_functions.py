...
class Button:
    ...
    def shift(self, dx: int, dy: int):
        topleft = point(self.x + self.width/2 + dx, self.y + self.height/2 + dy)
        self.toImage = Image(topleft, self.fileName, (self.width, self.height), 0)

class Enemy:
    def __init__(self, pos: point, Type: str, healthModifier: float):
        # ENEMY DATA
        self.pos = pos
        ...
        # STATS
        self.Health = int(enemyDict[Type]["Health"] * healthModifier)
        ...
        # IMAGE
        self.imageFile = Image(self.pos, enemyDict[Type]["imageFile"], enemyDict[Type]["resolution"], 0)
...
class structure:
    def __init__(self, gridPos: point, Type: str, level: int):
        ...
        self.type = Type
        self.repairModeOn = False

        # STATS
        self.Health = towerDict[Type]["level"+str(self.level)]["Health"]
        self.maxHealth = towerDict[Type]["level"+str(self.level)]["Health"]
    ...
    def updateTowerLevel(self):
        ...
        self.maxHealth = towerDict[self.type]["level"+str(self.level)]["Health"]
    ...
    def repair(self, money, gameTick, surface):
        repairIcon = Image(self.center, "repairMode.png", (50, 50), gameTick % 360)

        if money > 0 and self.repairModeOn and self.Health <= self.maxHealth:
            repairIcon.render(surface)
            if gameTick % 5 == 0:
                self.Health += 5
                return True
        else:
...
def findDirection(start: point, end: point):
    ...
    if distance == 0:
        distance = 0.001
    return [(end.x - start.x) / distance, (end.y - start.y) / distance]
...
def matrix(gridPos: point, size: int):
    ...
    for x in range(size):
        for y in range(size):
            coords.append(gridPos.moveCall_coord(x, y))
    return coords
